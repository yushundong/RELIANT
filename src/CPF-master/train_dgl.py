...
from utils.logger import get_logger, output_results
...
def arg_parse(parser):
    ...
    parser.add_argument('--framework', type=str, default='CPF', help="Store the knowledge for a KD framework ('CPF' or 'GraphAKD')")
...
def test(conf):
    ...
    return loss_test, acc_test, logp, model.emb
...
if __name__ == '__main__':
    ...
    # ----------set random seed for teacher model----------
    ...
    # loss_test, acc_test, logp = test(conf)
    loss_test, acc_test, logp, emb = test(conf)
    preds = logp.max(1)[1].type_as(labels).cpu().numpy()
    labels = labels.cpu().numpy()
    output = np.exp(logp.cpu().detach().numpy())
    acc_test = acc_test.cpu().item()
    loss_test = loss_test.cpu().item()

    # evaluate fairness
    if conf['dataset'] in ['bail', 'credit', 'raw', 'german', 'medium', 'small']:
        parity, equality = fair_metric(preds[idx_test.cpu().numpy()], labels[idx_test.cpu().numpy()],sens[idx_test.cpu().numpy()])
    
    if conf['framework'] == 'CPF':
        ...
        if 'a' in G.edata:
            ...
            sp_att = sp.coo_matrix((G.edata['a'].cpu().detach().numpy(), edge.cpu()), shape=[features.shape[0], features.shape[0]])
            sp.save_npz(output_dir.joinpath('attention_weight.npz'), sp_att, compressed=True)
    elif conf['framework'] == 'GraphAKD':
        torch.save({'logits': logp, 'embedding': emb, 'parity': parity, 'equality': equality}, '../GraphAKD/GraphAKD-main/distilled/'+conf['dataset']+'_'+conf['model_name']+'.pth.tar')
    else:
        print('Undefined framework.')
    results_dict = defaultdict(list)
    variants = list(gen_variants(dataset=[conf['dataset']], model=[conf['model_name']], seed=[conf['seed']]))
    results = dict(TestLoss=loss_test, TestAcc=acc_test, DeltaSP=parity, DeltaEO=equality)
    for variant in variants:
        results_dict[variant[:]] = [results]
        output_results(results_dict)

    # ----------optional : recording results----------
    # f = open(f"./{conf['dataset']}_{conf['model_name']}.txt", 'a')
    # s = f"TEACHER   loss:{loss_test} acc_test:{acc_test} parity:{parity} equality:{equality}"
    # f.write(s + '\n')
